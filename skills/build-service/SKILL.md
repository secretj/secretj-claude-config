...
- 그 외 → 일반 agent (`planner`, `developer`, `designer`, `infra`, `qa`, `security`)
...
### Phase 3 — 병렬 설계 (designer · developer · infra · security 동시)
**호출**: **단일 메시지에서 4개 Agent calls 병렬**

```
Agent(designer, "Phase 2 분해 결과의 화면 N개 명세 + 11 mistake 자가검증 + 차별화 가설표. <컨텍스트>")
Agent(developer, "Phase 2 분해 결과의 도메인 모델(DDD) + ADR + OpenAPI 발췌 + 도메인 다이어그램. <컨텍스트>. 이번 Phase는 설계만, 코드 구현은 Phase 5.")
...
**출력 수집**: 4 산출물 → `.build-service/{slug}/03-{designer|developer|infra|security}.md`
...
**프롬프트 골자**:
```
Phase 3 설계 + Phase 4 정합 통과한 명세를 코드로 구현해라.
...
.build-service/{slug}/
  ...
  01-prd.md            (planner)
  02-breakdown.md      (PM)
  03-designer.md / 03-developer.md / 03-infra.md / 03-security.md
